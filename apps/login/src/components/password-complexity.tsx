...
function CheckIcon({ title }: { title: string }) {
  return (
    <svg
      ...
      strokeWidth={1.5}
      stroke="currentColor"
      className="las la-check mr-2 h-6 w-6 flex-none text-lg text-green-500"
      role="img"
    >
      <title>{title}</title>
...
function CrossIcon({ title }: { title: string }) {
  return (
    <svg
      className="las la-times mr-2 h-6 w-6 flex-none text-lg text-warn-light-500"
      xmlns="http://www.w3.org/2000/svg"
      fill="none"
      viewBox="0 0 24 24"
      strokeWidth={1.5}
...
const desc = "text-14px leading-4 text-input-light-label";
